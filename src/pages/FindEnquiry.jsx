...
const FindEnquiry = () => {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      if (indentResult.success && indentResult.data && indentResult.data.length >= 7) {
        ...
        // Debug: Print headers to see column names
        console.log('INDENT Headers:', headers);
        ...
        console.log('Column Indices - Status:', statusIndex, 'Planned 2:', planned2Index, 'Actual 2:', actual2Index);
        ...
        const processedData = dataFromRow7
          .filter(row => {
            ...
            // Debug: Print values for each row
            console.log('Row values - Status:', status, 'Planned 2:', planned2, 'Actual 2:', actual2);

            // Updated condition: Column Q (Planned 2) not null/empty AND Column R (Actual 2) null/empty
            const isValidStatus = status === 'NeedMore';
            const hasPlanned2 = planned2 && String(planned2).trim() !== '';
            const noActual2 = !actual2 || String(actual2).trim() === '';

            console.log('Conditions - isValidStatus:', isValidStatus, 'hasPlanned2:', hasPlanned2, 'noActual2:', noActual2);
          ...
          .map(row => ({
            ...
            numberOfPost: row[6],                    // Column G (index 6, columns start from 0)
            competitionDate: row[7],                 // Column H (index 7)
            socialSite: row[getIndex('Social Site')],
            status: row[statusIndex],
            plannedDate: row[planned2Index],
        ...
        // DIRECTLY SET THE PROCESSED DATA - NO ADDITIONAL FILTERING
        ...
        if (enquiryResult.success && enquiryResult.data && enquiryResult.data.length > 0) {
          ...
          // If no ENQUIRY data, just show all processed data
          setIndentData(processedData);
        }
  ...
  const uploadFileToGoogleDrive = async (file, type) => {
    try {
      ...
      // Remove data URL prefix completely (data:image/png;base64,)
      const base64String = base64Data.includes(',')
        ? base64Data.split(',')[1]
        : base64Data;

      console.log('Cleaned base64 length:', base64String.length);
      ...
      console.log('Upload params:', {
        ...
        base64Length: uploadParams.base64Data.length
      });
